...
int gtf(float32_t a, float32_t b) {
    ...
    //asm("            call    _gtfp       ; call floating point library routine");
    ...
    asm("            lbz     gteq        ; if equal, load zero false");
    asm("            ldn     r7          ; check sign bit of result");
    asm("            shl                 ; put sign bit in DF");
    asm("            lbnf    gttrue      ; if sign bit is zero then true");
    asm("            ldi     000h        ; else load false value");
    asm("            lskp                ; jump over next two bytes");
    asm("gttrue:     ldi 0FFh            ; load true value");
    asm("gteq:       stxd                ; push value onto ESP ");
