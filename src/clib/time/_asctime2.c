...
char *_asctime2(struct tm *tp) {
    ...
    sprintf(buf, "%s %s %s%d %s%d:%s%d:%s%d %d\n",
        days + (tp->tm_wday)*4, months + (tp->tm_mon)*4,
        (tp->tm_mday < 10) ? " " : "", tp->tm_mday,
        (tp->tm_hour < 10) ? "0" : "", tp->tm_hour,
        (tp->tm_min < 10) ? "0" : "", tp->tm_min,
        (tp->tm_sec < 10) ? "0" : "", tp->tm_sec, tp->tm_year+1900);
