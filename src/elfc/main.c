...
static void compile(char *file, char *def) {
    ...
    if (file) {
        ...
        if ((in = fopen(file, "r")) == NULL) cmderror("no such file: %s", file);
        if (!O_testonly) {
...
static void collect(char *file, int temp) {
    ...
    if (Nf >= MAXFILES) cmderror("too many input files", NULL);
    Temp[Nf] = temp;
    Files[Nf++] = file;
}
...
static void assemble(char *file, char *path) {
    ...
    if (system(cmd)) cmderror("assembler invocation failed", NULL);
...
static void link(char *fname, char *path) {
  ...
  //grw - rewrote logic for Link/02
    ...
    if (O_verbose > 0) printf("%s\n", cmd);
    if (system(cmd)) cmderror("linker invocation failed", NULL);
}
...
static void usage(void) {
    ...
    //	" file [...]\n");
    printf(
        "Usage: elfc [-h] [-ctvLNSV] [-d opt] [-o file] [-D macro[=text]] file "
        "[...]\n");
}
...
int main(int argc, char *argv[]) {
    int i, j;
    char *def;
    // grw need to pass file name to Asm/02 and Link/02
    char *fname = "";
    ...
    while (i < argc) {
        if (filetype(argv[i]) == 'c') {
            ...
            fname = argv[i];
            if (Errors && !O_testonly) cmderror("compilation stopped", NULL);
            if (!O_asmonly && !O_testonly) assemble(argv[i], Fpath);
            i++;
            ...
            if (!exists(argv[i])) cmderror("no such file: %s", argv[i]);
            collect(argv[i++], 0);
        }
    }
