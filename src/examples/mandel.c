...
int main() {
    ...
    for (y = -HEIGHT / 2; y < HEIGHT / 2; y++) {
        for (x = -WIDTH / 2; x < WIDTH / 2; x++) {
            ...
            cr = subi32(divi32(muli32x16(i32_from_int(x), 3000),
                i32_from_int(WIDTH), NULL), i32_from_int(500));
            // ci = (y * 3000 / HEIGHT);      // -1500 to 1500
            ci = divi32(muli32x16(i32_from_int(y), 3000),
                i32_from_int(HEIGHT), NULL);
            zr = i32_from_int(0), zi = i32_from_int(0);
            
            for (i = 0; i < ITER; i++) {
                // Integer complex multiplication and addition
                // (zr+zi*i)^2 = zr^2 - zi^2 + 2*zr*zi*i
                // next_zr = (zr * zr - zi * zi) / SCALE + cr;
                next_zr = addi32(divi32(subi32(muli32(zr, zr),
                    muli32(zi, zi)), scale, NULL), cr);
                // next_zi = (2 * zr * zi) / SCALE + ci;
                next_zi = addi32(divi32(muli32x16(muli32(zr, zi), 2),
                    scale, NULL), ci);
                ...
                if (cmpi32(addi32(muli32(zr, zr), muli32(zi, zi)),
                    scale_squaredx4) == 1) {
                    break;
                }
            }
